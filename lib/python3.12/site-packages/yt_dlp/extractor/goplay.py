...
class GoPlayIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(www\.)?goplay\.be/video/([^/]+/[^/]+/|)(?P<display_id>[^/#]+)"
    )
    ...
    def _real_extract(self, url):
        ...
        video_data_json = self._html_search_regex(
            r'<div\s+data-hero="([^"]+)"', webpage, "video_data"
        )
        video_data = self._parse_json(unescapeHTML(video_data_json), display_id).get(
            "data"
        )
        ...
        api = self._download_json(
            ...
            video_id,
            headers={"Authorization": "Bearer %s" % self._id_token},
        )
...
class AwsIdp:
    ...
    def __get_challenge_response_request(self, challenge_parameters, password):
        ...
        hkdf = self.__get_hkdf_key_for_password(
            user_id_for_srp, password, self.__hex_to_long(srp_b), salt
        )
    ...
    def __get_current_timestamp():
        ...
        format_string = "{} {} {} %H:%M:%S UTC %Y".format(
            days[time_now.weekday()], months[time_now.month], time_now.day
        )
