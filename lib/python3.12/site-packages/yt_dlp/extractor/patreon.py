    ...
    def _call_api(self, ep, item_id, query=None, headers=None, fatal=True, note=None):
        ...
        try:
            return self._download_json(
                ...
                item_id,
                note="Downloading API JSON" if not note else note,
                query=query,
                fatal=fatal,
                headers=headers,
            )
    ...
    def _real_extract(self, url):
        ...
        for i in post.get("included", []):
            ...
            if i_type == "media":
                ...
                info.setdefault("tags", []).append(
                    traverse_obj(i, ("attributes", "value"))
                )
                ...
                info.update(
                    {
                        ...
                        "channel_follower_count": int_or_none(
                            traverse_obj(i, ("attributes", "patron_count"))
                        ),
                    }
                )
    ...
    def _get_comments(self, post_id):
        ...
        for page in itertools.count(1):
            ...
            response = self._call_api(
                ...
                post_id,
                query=params,
                note="Downloading comments page %d" % page,
            )

            cursor = None
            for comment in traverse_obj(
                response,
                (("data", ("included", lambda _, v: v["type"] == "comment")), ...),
            ):
                count += 1
...
class PatreonCampaignIE(PatreonBaseIE):
    ...
    _TESTS = [
        ...
        {"url": "https://www.patreon.com/dissonancepod/posts", "only_matching": True},
        {"url": "https://www.patreon.com/m/5932659", "only_matching": True},
    ]
    ...
    def _entries(self, campaign_id):
        ...
        for page in itertools.count(1):
            ...
            posts_json = self._call_api(
                ...
                campaign_id,
                query=params,
                note="Downloading posts page %d" % page,
            )
    ...
    def _real_extract(self, url):
        ...
        if campaign_id is None:
            webpage = self._download_webpage(
                url, vanity, headers={"User-Agent": self.USER_AGENT}
            )
            campaign_id = self._search_regex(
                r"https://www.patreon.com/api/campaigns/(\d+)/?", webpage, "Campaign ID"
            )
