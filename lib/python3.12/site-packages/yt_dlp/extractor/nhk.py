...
class NhkVodIE(NhkBaseIE):
    ...
    _VALID_URL = [
        rf"{NhkBaseIE._BASE_URL_REGEX}/(?P<type>video)/(?P<id>[0-9a-z]+)",
...
class NhkVodProgramIE(NhkBaseIE):
    ...
    def _real_extract(self, url):
        ...
        program_description = clean_html(
            get_element_by_class("p-programDetail__text", html)
        )
...
class NhkForSchoolSubjectIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://www\.nhk\.or\.jp/school/(?P<id>%s)/?(?:[\?#].*)?$"
    ...
    def _real_extract(self, url):
        ...
        if headline_id:
            return self._extract_episode_info(
                traverse_obj(
                    meta,
                    ("detail_list", lambda _, v: v["headline_id"] == headline_id),
                    get_all=False,
                ),
                programme_id,
                series_meta,
        ...
        return self.playlist_result(
            entries(),
            programme_id,
            playlist_description=meta.get("site_detail"),
            **series_meta,
        )
...
class NhkRadioNewsPageIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        return self.url_result(
            "https://www.nhk.or.jp/radio/ondemand/detail.html?p=F261_01", NhkRadiruIE
        )
    ...
    def _real_extract(self, url):
        ...
        if not data:
            raise ExtractorError(
                "Invalid area. Valid areas are: %s"
