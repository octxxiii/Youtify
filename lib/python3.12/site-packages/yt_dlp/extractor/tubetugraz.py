        ...
        for type in self._FORMAT_TYPES:
            if not has_hls:
                hls_formats = (
                    self._extract_m3u8_formats(
                        ...
                        "mp4",
                        fatal=False,
                        note=f"Downloading {type} HLS manifest",
                        errnote=False,
                    )
                    or []
                )
            ...
            if not has_dash:
                dash_formats = self._extract_mpd_formats(
                    ...
                    id,
                    fatal=False,
                    note=f"Downloading {type} DASH manifest",
                    errnote=False,
                )
...
class TubeTuGrazSeriesIE(TubeTuGrazBaseIE):
    ...
    def _real_extract(self, url):
        ...
        episodes_data = self._download_json(
            self._API_EPISODE, id, query={"sid": id}, note="Downloading episode list"
        )
        series_data = self._download_json(
            ...
            query={"seriesId": id, "count": 1, "sort": "TITLE"},
        )
