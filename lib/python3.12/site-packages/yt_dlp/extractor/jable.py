...
class JableIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        formats = self._extract_m3u8_formats(
            self._search_regex(r"var\s+hlsUrl\s*=\s*\'([^\']+)", webpage, "hls_url"),
            video_id,
            "mp4",
            m3u8_id="hls",
        )
        ...
        return {
            ...
            "upload_date": unified_strdate(
                self._search_regex(
                    r'class="inactive-color">\D+\s+(\d{4}-\d+-\d+)',
                    webpage,
                    "upload_date",
                    default=None,
                )
            ...
            "like_count": int_or_none(
                self._search_regex(
                    r'#icon-heart"></use></svg><span class="count">(\d+)',
                    webpage,
                    "link_count",
                    default=None,
                )
...
class JablePlaylistIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?jable\.tv/(?:categories|models|tags)/(?P<id>[\w-]+)"
    )
    _TESTS = [
        {
