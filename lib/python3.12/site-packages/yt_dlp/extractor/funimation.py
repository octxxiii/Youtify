...
class FunimationBaseIE(InfoExtractor):
    ...
    def _get_region(self):
        ...
        region = (
            region_cookie.value
            if region_cookie
            else self.get_param("geo_bypass_country")
        )
        return (
            region
            or traverse_obj(
                self._download_json(
                    "https://geo-service.prd.funimationsvc.com/geo/v1/region/check",
                    None,
                    fatal=False,
                    note="Checking geo-location",
                    errnote="Unable to fetch geo-location information",
                ),
                "region",
            )
            or "US"
...
class FunimationPageIE(FunimationBaseIE):
    ...
    def _real_extract(self, url):
        locale, show, episode = self._match_valid_url(url).group(
            "lang", "show", "episode"
        )
        ...
        return self.url_result(
            f"https://www.funimation.com/player/{video_id}",
            FunimationIE.ie_key(),
            video_id,
        )
...
class FunimationIE(FunimationBaseIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            ...
            note=f"Downloading player webpage for {initial_experience_id}",
        )
        episode, season, show = self._get_episode(
            webpage, experience_id=int(initial_experience_id)
        )
        episode_id = str(episode["episodePk"])
        display_id = episode.get("slug") or episode_id

        formats, subtitles, thumbnails, duration = [], {}, [], 0
        requested_languages, requested_versions = self._configuration_arg(
            "language"
        ), self._configuration_arg("version")
        language_preference = qualities((requested_languages or [""])[::-1])
        source_preference = qualities(
            (requested_versions or ["uncut", "simulcast"])[::-1]
        )
        only_initial_experience = "seperate-video-versions" in self.get_param(
            "compat_opts", []
        )
        ...
        for lang, version, fmt in self._get_experiences(episode):
            ...
            if (
                ...
                and experience_id != initial_experience_id
                or requested_languages
                and lang.lower() not in requested_languages
                or requested_versions
                and version.lower() not in requested_versions
            ):
                continue
            ...
            self.extract_subtitles(
                ...
                episode=(
                    episode if experience_id == initial_experience_id else episode_id
                ),
            )
            ...
            for source in sources:
                ...
                if source_type == "m3u8":
                    current_formats.extend(
                        self._extract_m3u8_formats(
                            source_url,
                            display_id,
                            "mp4",
                            m3u8_id="%s-%s" % (experience_id, "hls"),
                            fatal=False,
        ...
        if not formats and (requested_languages or requested_versions):
            self.raise_no_formats(
                "There are no video formats matching the requested languages/versions",
                expected=True,
                video_id=display_id,
            )
    ...
    def _get_subtitles(
        self, subtitles, experience_id, episode, display_id, format_name
    ):
        if isinstance(episode, str):
            webpage = self._download_webpage(
                f"https://www.funimation.com/player/{experience_id}/",
                display_id,
                fatal=False,
                note=f"Downloading player webpage for {format_name}",
            )
            ...
            for source in f.get("sources"):
                for text_track in source.get("textTracks"):
                    ...
                    current_sub = {
                        "url": text_track["src"],
                        "name": join_nonempty(
                            version, text_track.get("label"), sub_type, delim=" "
                        ),
                    }
                    lang = join_nonempty(
                        text_track.get("language", "und"),
...
class FunimationShowIE(FunimationBaseIE):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {"id": 1315000, "title": "SK8 the Infinity"},
            "playlist_count": 13,
            "params": {
                "skip_download": True,
            },
        },
        {
            ...
            "info_dict": {"id": 39643, "title": "Ouran High School Host Club"},
            "playlist_count": 26,
            "params": {
                "skip_download": True,
            },
    ...
    def _real_extract(self, url):
        ...
        show_info = self._download_json(
            ...
            % (display_id, self._REGION, locale or "en"),
            display_id,
        )
        ...
        vod_items = traverse_obj(
            items_info,
            ("items", ..., lambda k, _: re.match(r"(?i)mostRecent[AS]vod", k), "item"),
        )
        ...
        return {
            ...
            "entries": orderedSet(
                self.url_result(
                    "%s/%s" % (base_url, vod_item.get("episodeSlug")),
                    FunimationPageIE.ie_key(),
                    vod_item.get("episodeId"),
                    vod_item.get("episodeName"),
                )
                for vod_item in sorted(
                    vod_items, key=lambda x: x.get("episodeOrder", -1)
                )
            ),
        }
