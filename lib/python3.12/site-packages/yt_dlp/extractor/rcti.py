    ...
    def _real_extract(self, url):
        ...
        if is_upcoming:
            self.raise_no_formats(
                (
                    "This event will start at %s." % video_json["live_label"]
        ...
        if "akamaized" in video_url:
            ...
            conviva_json_res = self._download_json(
                ...
                fatal=False,
                data=json.dumps(conviva_json_data).encode("utf-8"),
            )
            if conviva_json_res and conviva_json_res.get("err") != "ok":
                self.report_warning(
                    "Conviva said: %s" % str(conviva_json_res.get("err"))
                )

        video_meta, meta_paths = self._call_api(
            "https://api.rctiplus.com/api/v1/%s/%s" % (video_type, video_id),
            display_id,
...
class RCTIPlusSeriesIE(RCTIPlusBaseIE):
    ...
    def _entries(
        ...
        total_pages = 0
        try:
            total_pages = self._call_api("%s&length=20&page=0" % url, display_id, note)[
                1
