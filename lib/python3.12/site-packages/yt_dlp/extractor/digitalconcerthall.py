...
class DigitalConcertHallIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        token_response = self._download_json(
            ...
            None,
            "Obtaining token",
            errnote="Unable to obtain token",
    ...
    def _entries(self, items, language, type_, **kwargs):
        for item in items:
            ...
            m3u8_url = traverse_obj(
                stream_info,
                ("channel", lambda k, _: k.startswith("vod_mixed"), "stream", 0, "url"),
                get_all=False,
            )
    ...
    def _real_extract(self, url):
        language, type_, video_id = self._match_valid_url(url).group(
            "language", "type", "id"
        )
        if not language:
            language = "en"
        ...
        vid_info = self._download_json(
            f"https://api.digitalconcerthall.com/v2/{type_}/{video_id}",
            video_id,
            headers={"Accept": "application/json", "Accept-Language": language},
        )
        album_artist = " / ".join(
            traverse_obj(vid_info, ("_links", "artist", ..., "name")) or ""
        )
        videos = (
            [vid_info]
            if type_ == "film"
