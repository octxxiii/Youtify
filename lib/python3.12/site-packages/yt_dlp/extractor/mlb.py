...
class MLBTVIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        entitlement = self._download_webpage(
            f"https://media-entitlement.mlb.com/api/v3/jwt?os=Android&appname=AtBat&did={str(uuid.uuid4())}",
            None,
            headers={
                "User-Agent": "okhttp/3.12.1",
        ...
        self._access_token = self._download_json(
            "https://us.edge.bamgrid.com/token",
            None,
            headers={
    ...
    def _real_extract(self, url):
        ...
        for airing in airings:
            ...
            f, s = self._extract_m3u8_formats_and_subtitles(
                ...
                video_id,
                "mp4",
                m3u8_id=join_nonempty(
                    airing.get("feedType"), airing.get("feedLanguage")
                ),
            )
            formats.extend(f)
        ...
        return {
            "id": video_id,
            "title": traverse_obj(
                airings, (..., "titles", 0, "episodeName"), get_all=False
            ),
            "is_live": traverse_obj(
                airings, (..., "mediaConfig", "productType"), get_all=False
            )
            == "LIVE",
            "formats": formats,
            "subtitles": subtitles,
...
class MLBArticleIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        content_data_id = traverse_obj(
            apollo_cache_json,
            ("ROOT_QUERY", lambda k, _: k.startswith("getForgeContent"), "id"),
            get_all=False,
        )
        ...
        return self.playlist_from_matches(
            traverse_obj(
                content_real_info,
                ("parts", lambda _, v: v["typename"] == "Video", "id"),
            ),
            getter=lambda x: f'https://www.mlb.com/video/{apollo_cache_json[x]["slug"]}',
            ie=MLBVideoIE,
            playlist_id=content_real_info.get("_translationId"),
