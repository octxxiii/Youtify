...
class YleAreenaIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        description = traverse_obj(
            video_data,
            ("data", "ongoing_ondemand", "description", "fin"),
            expected_type=str,
        )

        subtitles = {}
        for sub in traverse_obj(
            video_data, ("data", "ongoing_ondemand", "subtitles", ...)
        ):
        ...
        kaltura_id = traverse_obj(
            video_data, ("data", "ongoing_ondemand", "kaltura", "id"), expected_type=str
        )
        if kaltura_id:
            info_dict = {
            ...
            info_dict = {
                ...
                "formats": self._extract_m3u8_formats(
                    video_data["data"]["ongoing_ondemand"]["manifest_url"],
                    video_id,
                    "mp4",
                    m3u8_id="hls",
                ),
        ...
        return {
            **info_dict,
            "title": (
                traverse_obj(
                    video_data,
                    ("data", "ongoing_ondemand", "title", "fin"),
                    expected_type=str,
                )
                or episode
                or info.get("title")
            ...
            "series": (
                traverse_obj(
                    video_data,
                    ("data", "ongoing_ondemand", "series", "title", "fin"),
                    expected_type=str,
                )
                or series
            ),
            ...
            "episode_number": (
                traverse_obj(
                    video_data,
                    ("data", "ongoing_ondemand", "episode_number"),
            ...
            "age_limit": traverse_obj(
                video_data,
                ("data", "ongoing_ondemand", "content_rating", "age_restriction"),
                expected_type=int_or_none,
            ),
            "subtitles": subtitles,
            "release_date": unified_strdate(
                traverse_obj(
                    video_data,
                    ("data", "ongoing_ondemand", "start_time"),
                    expected_type=str,
                )
            ),
        }
