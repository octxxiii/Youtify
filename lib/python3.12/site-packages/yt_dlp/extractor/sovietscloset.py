...
class SovietsClosetIE(SovietsClosetBaseIE):
    ...
    def _extract_bunnycdn_iframe(self, video_id, bunnycdn_id):
        iframe = self._download_webpage(
            ...
            video_id,
            note="Downloading BunnyCDN iframe",
            headers=self.MEDIADELIVERY_REFERER,
        )

        m3u8_url = self._search_regex(r"(https?://.*?\.m3u8)", iframe, "m3u8 url")
        thumbnail_url = self._search_regex(
            r"(https?://.*?thumbnail\.jpg)", iframe, "thumbnail url"
        )

        m3u8_formats = self._extract_m3u8_formats(
            m3u8_url, video_id, headers=self.MEDIADELIVERY_REFERER
        )
