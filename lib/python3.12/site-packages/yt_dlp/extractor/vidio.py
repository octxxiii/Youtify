    ...
    def _perform_login(self, username, password):
        ...
        if login_post_urlh.status == 401:
            ...
            if "Akun terhubung ke" in reason:
                ...
                raise ExtractorError(
                    "Unable to log in: %s. %s" % (reason, clean_html(subreason)),
                    expected=True,
                )
    ...
    def _real_extract(self, url):
        ...
        if is_premium:
            sources = self._download_json(
                "https://www.vidio.com/interactions_stream.json?video_id=%s&type=videos"
                % video_id,
                display_id,
    ...
    def _playlist_entries(self, playlist_url, display_id):
        ...
        while playlist_url:
            playlist_json = self._call_api(
                playlist_url, display_id, "Downloading API JSON page %s" % index
            )
...
class VidioLiveIE(VidioBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if stream_meta.get("is_premium"):
            sources = self._download_json(
                "https://www.vidio.com/interactions_stream.json?video_id=%s&type=livestreamings"
                % video_id,
                display_id,
                note="Downloading premier API JSON",
            )
