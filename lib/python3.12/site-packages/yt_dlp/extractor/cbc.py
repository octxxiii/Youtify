...
class CBCGemIE(InfoExtractor):
    IE_NAME = "gem.cbc.ca"
    _VALID_URL = (
        r"https?://gem\.cbc\.ca/(?:media/)?(?P<id>[0-9a-z-]+/s[0-9]+[a-z][0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _find_secret_formats(self, formats, video_id):
        ...
        base_url = re.sub(
            r"(Manifest\(.*?),filter=[\w-]+(.*?\))", r"\1\2", base_format["url"]
        )
        url = re.sub(r"(Manifest\(.*?),format=[\w-]+(.*?\))", r"\1\2", base_url)

        secret_xml = self._download_xml(
            url, video_id, note="Downloading secret XML", fatal=False
        )
            ...
            for video_quality in child:
                ...
                yield {
                    **base_format,
                    ...
                    "url": re.sub(
                        r"(QualityLevels\()\d+(\))", rf"\g<1>{bitrate}\2", base_url
                    ),
    ...
    def _real_extract(self, url):
        ...
        m3u8_info = self._download_json(
            video_info["playSession"]["url"], video_id, headers=headers
        )
...
class CBCGemPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        show_info = self._download_json(
            self._API_BASE + show, season_id, expected_status=426
        )
        season = int(match.group("season"))

        season_info = next(
            (s for s in show_info["seasons"] if s.get("season") == season), None
        )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "formats": self._extract_m3u8_formats(
                stream_data["url"], video_id, "mp4", live=True
            ),
            "is_live": True,
            **traverse_obj(
                video_info,
