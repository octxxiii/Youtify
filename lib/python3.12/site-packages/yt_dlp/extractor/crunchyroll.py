...
class CrunchyrollBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        login_response = self._download_json(
            f"{self._API_BASE}/login.1.json",
            None,
            "Logging in",
            data=urlencode_postdata(
                {"account": username, "password": password, "session_id": session_id}
            ),
        )
        if login_response["code"] != "ok":
            raise ExtractorError(
                "Login failed. Server message: %s" % login_response["message"],
                expected=True,
            )
    ...
    def _update_auth(self):
        if (
            CrunchyrollBaseIE._AUTH_HEADERS
            and CrunchyrollBaseIE._AUTH_REFRESH > time_seconds()
        ):
            return
        ...
        if not CrunchyrollBaseIE._BASIC_AUTH:
            ...
            CrunchyrollBaseIE._BASIC_AUTH = (
                "Basic " + base64.b64encode(f"{cx_api_param}:".encode()).decode()
            )
        ...
        try:
            auth_response = self._download_json(
                ...
                None,
                note=f"Authenticating with grant_type={grant_type}",
                headers={"Authorization": CrunchyrollBaseIE._BASIC_AUTH},
            ...
            if isinstance(error.cause, HTTPError) and error.cause.status == 403:
                raise ExtractorError(
                    ...
                    "and your browser's User-Agent (with --user-agent)",
                    expected=True,
                )
            raise
        ...
        CrunchyrollBaseIE._AUTH_REFRESH = time_seconds(
            seconds=traverse_obj(
                auth_response, ("expires_in", {float_or_none}), default=300
            )
            - 10
        )

    def _locale_from_language(self, language):
        config_locale = self._configuration_arg(
            "metadata", ie_key=CrunchyrollBetaIE, casesense=True
        )
    ...
    def _call_base_api(self, endpoint, internal_id, lang, note=None, query={}):
        ...
        return self._download_json(
            ...
            internal_id,
            note or f"Calling API: {endpoint}",
    ...
    def _call_api(self, path, internal_id, lang, note="api", query={}):
        ...
        try:
            result = self._call_base_api(
                ...
                internal_id,
                lang,
                f"Downloading {note} JSON ({self._API_ENDPOINT})",
                query=query,
            )
        except ExtractorError as error:
    ...
    def _extract_formats(self, stream_response, display_id=None):
        ...
        for stream_type, streams in traverse_obj(
            ...
            for stream in traverse_obj(streams, lambda _, v: v["url"]):
                ...
                format_id = join_nonempty(
                    stream_type, format_field(stream, "hardsub_locale", "hardsub-%s")
                )
                available_formats[hardsub_lang] = (
                    stream_type,
                    format_id,
                    hardsub_lang,
                    stream["url"],
        ...
        requested_hardsubs = [
            ...
            for val in (self._configuration_arg("hardsub") or ["none"])
        ]
        ...
        audio_locale = traverse_obj(
            stream_response, ((None, "meta"), "audio_locale"), get_all=False
        )
            ...
            if stream_type.endswith("hls"):
                if hardsub_lang.lower() in full_format_langs:
                    adaptive_formats = self._extract_m3u8_formats(
                        ...
                        m3u8_id=format_id,
                        fatal=False,
                        note=f"Downloading {format_id} HLS manifest",
                    )
                else:
                    adaptive_formats = (
                        self._m3u8_meta_format(
                            stream_url, ext="mp4", m3u8_id=format_id
                        ),
                    )
                ...
                adaptive_formats = self._extract_mpd_formats(
                    ...
                    mpd_id=format_id,
                    fatal=False,
                    note=f"Downloading {format_id} MPD manifest",
                )
            else:
                self.report_warning(
                    f"Encountered unknown stream_type: {stream_type!r}",
                    display_id,
                    only_once=True,
                )
                continue
    ...
    def _extract_subtitles(self, data):
        ...
        for locale, subtitle in traverse_obj(
            data, ((None, "meta"), "subtitles", {dict.items}, ...)
        ):
            subtitles[locale] = [
                traverse_obj(subtitle, {"url": "url", "ext": "format"})
            ]
...
class CrunchyrollCmsBaseIE(CrunchyrollBaseIE):
    ...
    def _call_cms_api_signed(self, path, internal_id, lang, note="api"):
        if (
            not CrunchyrollCmsBaseIE._CMS_EXPIRY
            or CrunchyrollCmsBaseIE._CMS_EXPIRY <= time_seconds()
        ):
            response = self._call_base_api(
                "index/v2", None, lang, "Retrieving signed policy"
            )["cms_web"]
        ...
        return self._call_base_api(
            ...
            internal_id,
            lang,
            f"Downloading {note} JSON (signed cms)",
            query=CrunchyrollCmsBaseIE._CMS_QUERY,
        )
...
class CrunchyrollBetaIE(CrunchyrollCmsBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if object_type == "episode":
            ...
            first_movie_id = traverse_obj(
                response, ("movie_listing_metadata", "first_movie_id")
            )
            if not self._yes_playlist(internal_id, first_movie_id):
                return self.url_result(
                    f"{self._BASE_URL}/{lang}watch/{first_movie_id}",
                    CrunchyrollBetaIE,
                    first_movie_id,
                )

            def entries():
                movies = self._call_api(
                    f"movie_listings/{internal_id}/movies",
                    internal_id,
                    lang,
                    "movie list",
                )
                for movie_response in traverse_obj(movies, ("data", ...)):
                    yield self.url_result(
                        ...
                        CrunchyrollBetaIE,
                        **self._transform_movie_response(movie_response),
                    )

            return self.playlist_result(
                entries(), **self._transform_movie_response(response)
            )
        ...
        if not streams_link and traverse_obj(
            response, (f"{object_type}_metadata", "is_premium_only")
        ):
        ...
        stream_response = self._call_cms_api_signed(
            remove_start(streams_link, "/content/v2/cms/"),
            internal_id,
            lang,
            "stream info",
        )
    ...
    def _transform_episode_response(data):
        metadata = (
            traverse_obj(data, (("episode_metadata", None), {dict}), get_all=False)
            or {}
        )
        return {
            "id": data["id"],
...
class CrunchyrollBetaShowIE(CrunchyrollCmsBaseIE):
    ...
    def _real_extract(self, url):
        ...
        def entries():
            seasons_response = self._call_cms_api_signed(
                f"seasons?series_id={internal_id}", internal_id, lang, "seasons"
            )
            for season in traverse_obj(seasons_response, ("items", ..., {dict})):
                episodes_response = self._call_cms_api_signed(
                    f'episodes?season_id={season["id"]}',
                    season["id"],
                    lang,
                    "episode list",
                )
                for episode_response in traverse_obj(
                    episodes_response, ("items", ..., {dict})
                ):
                    yield self.url_result(
                        f'{self._BASE_URL}/{lang}watch/{episode_response["id"]}',
                        CrunchyrollBetaIE,
                        **CrunchyrollBetaIE._transform_episode_response(
                            episode_response
                        ),
                    )
        ...
        return self.playlist_result(
            ...
            internal_id,
            **traverse_obj(
                self._call_api(f"series/{internal_id}", internal_id, lang, "series"),
                (
                    "data",
                    0,
                    {
...
class CrunchyrollMusicIE(CrunchyrollBaseIE):
    ...
    def _real_extract(self, url):
        lang, internal_id, object_type = self._match_valid_url(url).group(
            "lang", "id", "type"
        )
        path, name = {
        ...
        response = traverse_obj(
            self._call_api(f"{path}/{internal_id}", internal_id, lang, name),
            ("data", 0, {dict}),
        )
        if not response:
            raise ExtractorError(
        ...
        if not streams_link and response.get("isPremiumOnly"):
            message = (
                f'This {response.get("type") or "media"} is for premium members only'
            )
            if self.is_logged_in:
                raise ExtractorError(message, expected=True)
            self.raise_login_required(message)
...
class CrunchyrollArtistIE(CrunchyrollBaseIE):
    ...
    def _real_extract(self, url):
        lang, internal_id = self._match_valid_url(url).group("lang", "id")
        response = traverse_obj(
            self._call_api(f"artists/{internal_id}", internal_id, lang, "artist info"),
            ("data", 0),
        )
        ...
        def entries():
            for attribute, path in [("concerts", "concert"), ("videos", "musicvideo")]:
                for internal_id in traverse_obj(response, (attribute, ...)):
                    yield self.url_result(
                        f"{self._BASE_URL}/watch/{path}/{internal_id}",
                        CrunchyrollMusicIE,
                        internal_id,
                    )

        return self.playlist_result(
            entries(), **self._transform_artist_response(response)
        )
