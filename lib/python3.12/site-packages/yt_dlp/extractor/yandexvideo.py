...
class ZenYandexIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_json = try_get(
            data_json, lambda x: x[serverstate]["exportData"]["video"], dict
        )
...
class ZenYandexChannelIE(InfoExtractor):
    ...
    def _entries(self, item_id, server_state_json, server_settings_json):
        items = traverse_obj(server_state_json, ("feed", "items", ...)) or traverse_obj(
            server_settings_json, ("exportData", "items", ...)
        )

        more = traverse_obj(server_state_json, ("links", "more")) or traverse_obj(
            server_settings_json, ("exportData", "more", "link")
        )
    ...
    def _real_extract(self, url):
        ...
        server_settings_json = traverse_obj(
            data, lambda k, _: k.startswith("__serverSettings__"), get_all=False
        )
