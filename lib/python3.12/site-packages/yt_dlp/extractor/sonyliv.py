...
class SonyLIVSeriesIE(InfoExtractor):
    ...
    def _entries(self, show_id):
        ...
        seasons = try_get(
            self._download_json(
                self._API_SHOW_URL.format(show_id), video_id=show_id, headers=headers
            ),
            lambda x: x["resultObj"]["containers"][0]["containers"],
            list,
        )
        for season in seasons or []:
            ...
            episodes = try_get(
                self._download_json(
                    self._API_EPISODES_URL.format(season_id),
                    video_id=season_id,
                    headers=headers,
                ),
                lambda x: x["resultObj"]["containers"][0]["containers"],
            ...
            for episode in episodes or []:
                ...
                yield self.url_result(
                    "sonyliv:%s" % video_id, ie=SonyLIVIE.ie_key(), video_id=video_id
                )
