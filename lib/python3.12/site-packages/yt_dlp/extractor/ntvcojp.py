...
class NTVCoJpCUIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        account_id = (
            traverse_obj(player_config, ("player", "account")) or "3855502814001"
        )
        title = traverse_obj(player_config, ("movie", "name"))
        if not title:
            og_title = self._og_search_title(webpage, fatal=False) or traverse_obj(
                player_config, ("player", "title")
            )
            if og_title:
