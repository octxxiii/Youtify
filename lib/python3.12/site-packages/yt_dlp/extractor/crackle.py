...
class CrackleIE(InfoExtractor):
    ...
    def _download_json(self, url, *args, **kwargs):
        ...
        h = (
            hmac.new(
                b"IGSLUQCBDFHEOIFM", "|".join([url, timestamp]).encode(), hashlib.sha1
            )
            .hexdigest()
            .upper()
        )
        ...
        for num, country in enumerate(countries):
            if num == 1:  # start hard-coded list
                self.report_warning(
                    ...
                    % (
                        "Unable to obtain video formats from %s API"
                        % geo_bypass_country
                ...
                if country in countries:
                    self.to_screen(
                        "Downloading from %s API was already attempted. Skipping..."
                        % country
                    )
                    continue
            ...
            try:
                media = self._download_json(
                    ...
                    % (video_id, country),
                    video_id,
                    note="Downloading media JSON from %s API" % country,
