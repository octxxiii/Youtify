...
class PokerGoIE(PokerGoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        data_json = self._download_json(
            f"https://api.pokergo.com/v2/properties/{self._PROPERTY_ID}/videos/{id}",
            id,
        ...
        series_json = (
            next(
                dct
                for dct in data_json.get("show_tags") or []
                if dct.get("video_id") == id
            )
            or {}
        )
...
class PokerGoCollectionIE(PokerGoBaseIE):
    ...
    def _entries(self, id):
        data_json = self._download_json(
            f"https://api.pokergo.com/v2/properties/{self._PROPERTY_ID}/collections/{id}?include=entities",
            id,
            headers={"authorization": f"Bearer {self._AUTH_TOKEN}"},
        )["data"]
