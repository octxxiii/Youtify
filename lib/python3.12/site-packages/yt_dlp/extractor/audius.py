...
class AudiusBaseIE(InfoExtractor):
        ...
        if isinstance(response, dict):
            ...
            if len(response) == 1 and "message" in response:
                raise ExtractorError(
                    "API error: %s" % response["message"], expected=True
                )
    ...
    def _api_request(
        ...
        try:
            response = super(AudiusBaseIE, self)._download_json(
                "%s%s%s" % (self._API_BASE, self._API_V, path),
                item_id,
                note=note,
                errnote=errnote,
...
class AudiusTrackIE(AudiusIE):  # XXX: Do not subclass from concrete IE
    ...
    _TESTS = [
        {"url": "audius:9RWlo", "only_matching": True},
        {
...
class AudiusPlaylistIE(AudiusBaseIE):
    ...
    def _build_playlist(self, tracks):
        ...
        for track in tracks:
            ...
            entries.append(
                self.url_result(
                    "audius:%s" % track_id, ie=AudiusTrackIE.ie_key(), video_id=track_id
                )
            )
        return entries
...
class AudiusProfileIE(AudiusPlaylistIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        api_call = self._api_request(
            "/full/users/handle/%s/tracks" % profile_id, profile_id
        )
        return self.playlist_result(
