...
class TubiTvIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_data = self._download_json(
            f"https://tubitv.com/oz/videos/{video_id}/content",
            video_id,
            query={
                "video_resources": [
                    "dash",
...
class TubiTvShowIE(InfoExtractor):
    ...
    def _entries(self, show_url, show_name):
        ...
        show_json = self._parse_json(
            self._search_regex(
                r"window\.__data\s*=\s*({[^<]+});\s*</script>", show_webpage, "data"
            ),
            show_name,
        ...
        for episode_id in show_json["fullContentById"].keys():
            ...
            yield self.url_result(
                "tubitv:%s" % episode_id, ie=TubiTvIE.ie_key(), video_id=episode_id
            )

    def _real_extract(self, url):
        show_name = self._match_valid_url(url).group("show_name")
        return self.playlist_result(
            self._entries(url, show_name), playlist_id=show_name
        )
