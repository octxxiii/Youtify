...
class MildomBaseIE(InfoExtractor):
    ...
    def _call_api(
        self, url, video_id, query=None, note="Downloading JSON metadata", body=None
    ):
...
class MildomIE(MildomBaseIE):
    ...
    def _real_extract(self, url):
        ...
        enterstudio = self._call_api(
            ...
            video_id,
            note="Downloading live metadata",
        ...
        servers = self._call_api(
            ...
            result_video_id,
            note="Downloading live server list",
            query={
                "user_id": video_id,
        ...
        playback_token = self._call_api(
            ...
            result_video_id,
            note="Obtaining live playback token",
            body={"host_id": video_id, "type": "hls"},
        )
        playback_token = traverse_obj(
            playback_token, ("data", ..., "token"), get_all=False
        )
        ...
        return {
            ...
            or traverse_obj(enterstudio, "anchor_intro"),
            "description": traverse_obj(
                enterstudio, "intro", "live_intro", expected_type=str
            ),
            "timestamp": float_or_none(enterstudio.get("live_start_ms"), scale=1000),
            "uploader": self._html_search_meta("twitter:title", webpage, default=None)
            or traverse_obj(enterstudio, "loginname"),
...
class MildomVodIE(MildomBaseIE):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f"https://www.mildom.com/playback/{user_id}/{video_id}", video_id
        )
            ...
            formats.append(
                {
                    ...
                    "protocol": "m3u8_native",
                    "width": fmt["level"]
                    * autoplay["video_width"]
                    // autoplay["video_height"],
                    "height": fmt["level"],
                    "vcodec": "h264",
                    "acodec": "aac",
                    "ext": "mp4",
...
class MildomClipIE(MildomBaseIE):
    ...
    def _real_extract(self, url):
        user_id, video_id = self._match_valid_url(url).group("user_id", "id")
        webpage = self._download_webpage(
            f"https://www.mildom.com/clip/{video_id}", video_id
        )

        clip_detail = self._call_api(
            "https://cloudac-cf-jp.mildom.com/nonolive/videocontent/clip/detail",
            video_id,
            note="Downloading playback metadata",
            query={
                "clip_id": video_id,
...
class MildomUserVodIE(MildomBaseIE):
    ...
    def _real_extract(self, url):
        ...
        self.to_screen(
            'This will download all VODs belonging to user. To download ongoing live video, use "https://www.mildom.com/%s" instead'
            % user_id
        )
        ...
        return self.playlist_result(
            ...
            user_id,
            f'Uploads from {profile["loginname"]}',
        )
