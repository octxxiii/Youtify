...
class BilibiliBaseIE(InfoExtractor):
    ...
    def extract_formats(self, play_info):
        ...
        missing_formats = format_names.keys() - set(
            traverse_obj(formats, (..., "quality"))
        )
        if missing_formats:
            self.to_screen(
                f'Format(s) {", ".join(format_names[i] for i in missing_formats)} are missing; '
    ...
    def _get_subtitles(self, video_id, cid, aid=None):
        ...
        subtitle_info = traverse_obj(
            self._download_json(
                "https://api.bilibili.com/x/player/v2",
                video_id,
                query=(
                    {"aid": aid, "cid": cid} if aid else {"bvid": video_id, "cid": cid}
                ),
                note=f"Extracting subtitle info {cid}",
            ),
            ("data", "subtitle"),
        )
        subs_list = traverse_obj(
            subtitle_info, ("subtitles", lambda _, v: v["subtitle_url"] and v["lan"])
        )
        if not subs_list and traverse_obj(subtitle_info, "allow_submit"):
            if not self._get_cookies("https://api.bilibili.com").get(
                "SESSDATA"
            ):  # no login session cookie
                self.report_warning(
                    f"CC subtitles (if any) are only visible when logged in. {self._login_hint()}",
                    only_once=True,
                )
        for s in subs_list:
    ...
    def _real_extract(self, url):
        ...
        if is_festival:
            ...
            if not play_info:
                if traverse_obj(play_info_obj, "code") == 87007:
                    ...
                    raise ExtractorError(
                        f"This is a supporter-only video: {msg}. {self._login_hint()}",
                        expected=True,
                    )
        ...
        if is_interactive:
            return self.playlist_result(
                self._get_interactive_entries(video_id, cid, metainfo),
                **metainfo,
                **{
...
class BiliBiliBangumiIE(BilibiliBaseIE):
    ...
    def _real_extract(self, url):
        ...
        play_info = self._download_json(
            ...
            episode_id,
            "Extracting episode",
            query={"fnval": "4048", "ep_id": episode_id},
            headers=headers,
        )
        ...
        formats = self.extract_formats(play_info)
        if not formats and (
            premium_only or "成为大会员抢先看" in webpage or "开通大会员观看" in webpage
        ):
            self.raise_login_required("This video is for premium members only")
...
class BilibiliSpaceVideoIE(BilibiliSpaceBaseIE):
    _VALID_URL = (
        r"https?://space\.bilibili\.com/(?P<id>\d+)(?P<video>/video)?/?(?:[?#]|$)"
    )
    _TESTS = [
        {
    ...
    def _extract_signature(self, playlist_id):
        session_data = self._download_json(
            "https://api.bilibili.com/x/web-interface/nav", playlist_id, fatal=False
        )

        key_from_url = lambda x: x[x.rfind("/") + 1 :].split(".")[0]
        img_key = (
            traverse_obj(session_data, ("data", "wbi_img", "img_url", {key_from_url}))
            or "34478ba821254d9d93542680e3b86100"
        )
        sub_key = (
            traverse_obj(session_data, ("data", "wbi_img", "sub_url", {key_from_url}))
    ...
    def _real_extract(self, url):
        ...
        def fetch_page(page_idx):
            ...
            query["w_rid"] = hashlib.md5(
                f"{urllib.parse.urlencode(query)}{signature}".encode()
            ).hexdigest()
            ...
            try:
                ...
                if isinstance(e.cause, HTTPError) and e.cause.status == 412:
                    raise ExtractorError(
                        "Request is blocked by server (412), please add cookies, wait and try later.",
                        expected=True,
                    )
                raise
            if response["code"] == -401:
                raise ExtractorError(
                    "Request is blocked by server (401), please add cookies, wait and try later.",
                    expected=True,
                )
...
class BilibiliCategoryIE(InfoExtractor):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {"id": "kichiku: mad", "title": "kichiku: mad"},
            "playlist_mincount": 45,
            "params": {"playlistend": 45},
        }
    ]
    ...
    def _fetch_page(self, api_url, num_pages, query, page_num):
        ...
        for video in video_list:
            yield self.url_result(
                "https://www.bilibili.com/video/%s" % video["bvid"],
                "BiliBili",
                video["bvid"],
            )
...
class BiliIntlBaseIE(InfoExtractor):
    ...
    def _get_subtitles(self, *, ep_id=None, aid=None):
        ...
        for sub in sub_json.get("subtitles") or []:
            ...
            sub_data = self._download_json(
                ...
                fatal=False,
                note=(
                    "Downloading subtitles%s" % f' for {sub["lang"]}'
                    if sub.get("lang")
                    else ""
                ),
            )
            ...
            subtitles.setdefault(sub.get("lang_key", "en"), []).append(
                {"ext": "srt", "data": self.json2srt(sub_data)}
            )
        return subtitles
    ...
    def _perform_login(self, username, password):
        ...
        password_hash = Cryptodome.PKCS1_v1_5.new(public_key).encrypt(
            (key_data["hash"] + password).encode("utf-8")
        )
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "chapters": [
                    ...
                    {"start_time": 76.242, "end_time": 161.161, "title": "Intro"},
                    {"start_time": 1325.742, "end_time": 1403.903, "title": "Outro"},
                ],
            },
        },
        {
            ...
            "info_dict": {
                ...
                "chapters": [
                    ...
                    {"start_time": 88.0, "end_time": 156.0, "title": "Intro"},
                    {"start_time": 1173.0, "end_time": 1259.535, "title": "Outro"},
                ],
            },
        },
        {
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "chapters": [
                    ...
                    {"start_time": 61.0, "end_time": 134.0, "title": "Intro"},
                    {"start_time": 1290.0, "end_time": 1379.0, "title": "Outro"},
                ],
            },
...
class BiliLiveIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        {"url": "https://live.bilibili.com/blanc/196", "only_matching": True},
    ]
