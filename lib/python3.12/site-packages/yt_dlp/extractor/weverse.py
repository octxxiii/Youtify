...
class WeverseBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        valid_username = traverse_obj(
            self._download_json(
                f"{self._ACCOUNT_API_BASE}/signup/email/status",
                None,
        ...
        try:
            auth = self._download_json(
                f"{self._ACCOUNT_API_BASE}/auth/token/by-credentials",
                None,
                data=json.dumps(
                    {
                        "email": username,
    ...
    def _real_initialize(self):
        ...
        token = try_call(
            lambda: self._get_cookies("https://weverse.io/")["we2_access_token"].value
        )
        if token:
    ...
    def _call_api(self, ep, video_id, data=None, note="Downloading API JSON"):
        ...
        wmd = base64.b64encode(
            hmac.HMAC(
                key, f"{api_path[:255]}{wmsgpad}".encode(), digestmod=hashlib.sha1
            ).digest()
        ).decode()
    ...
    def _real_extract(self, url):
        ...
        if live_status == "is_upcoming":
            ...
            in_key = self._call_api(
                f"/video/v1.0/vod/{api_video_id}/inKey?preview=false",
                video_id,
                data=b"{}",
