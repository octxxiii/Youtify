...
class NineNowIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        brightcove_id = (
            try_get(
                common_data, lambda x: x["episode"]["video"]["brightcoveId"], compat_str
            )
            or "ref:%s" % common_data["episode"]["video"]["referenceId"]
        )
        video_id = (
            str_or_none(try_get(common_data, lambda x: x["episode"]["video"]["id"]))
            or brightcove_id
        )
        ...
        timestamp = unified_timestamp(
            try_get(common_data, lambda x: x["episode"]["airDate"], compat_str)
        )
        release_date = unified_strdate(
            try_get(common_data, lambda x: x["episode"]["availability"], compat_str)
        )
        thumbnails_data = (
            try_get(common_data, lambda x: x["episode"]["image"]["sizes"], dict) or {}
        )
        thumbnails = [
            {
                "id": thumbnail_id,
