...
class LinkedInIE(LinkedInBaseIE):
    ...
    def _real_extract(self, url):
        ...
        description = clean_html(
            get_element_by_class("share-update-card__update-text", webpage)
        )
        like_count = int_or_none(
            get_element_by_class(
                "social-counts-reactions__social-counts-numRections", webpage
            )
        )
        creator = strip_or_none(
            clean_html(get_element_by_class("comment__actor-name", webpage))
        )

        sources = self._parse_json(
            extract_attributes(self._search_regex(r"(<video[^>]+>)", webpage, "video"))[
                "data-sources"
            ],
            video_id,
        )
...
class LinkedInLearningIE(LinkedInLearningBaseIE):
    ...
    def json2srt(self, transcript_lines, duration=None):
        ...
        for line, (line_dict, next_dict) in enumerate(
            zip_longest(transcript_lines, transcript_lines[1:])
        ):
            start_time, caption = (
            ...
            srt_data += "%d\n%s --> %s\n%s\n\n" % (
                line + 1,
                srt_subtitles_timecode(start_time),
    ...
    def _real_extract(self, url):
        ...
        if transcript_lines:
            subtitles["en"] = [
                {"ext": "srt", "data": self.json2srt(transcript_lines, duration)}
            ]
