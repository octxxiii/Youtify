...
class RTVEALaCartaIE(InfoExtractor):
    ...
    def _real_initialize(self):
        user_agent_b64 = base64.b64encode(
            self.get_param("http_headers")["User-Agent"].encode("utf-8")
        ).decode("utf-8")
...
class RTVEAudioIE(RTVEALaCartaIE):  # XXX: Do not subclass from concrete IE
    ...
    IE_DESC = "RTVE audio"
    _VALID_URL = (
        r"https?://(?:www\.)?rtve\.es/(alacarta|play)/audios/[^/]+/[^/]+/(?P<id>[0-9]+)"
    )
    ...
    def _extract_png_formats(self, audio_id):
        ...
        for quality, audio_url in self._decrypt_url(png):
            ...
            if ext == "m3u8":
                ...
                formats.extend(
                    self._extract_mpd_formats(audio_url, audio_id, "dash", fatal=False)
                )
            else:
                formats.append(
                    {
    ...
    def _real_extract(self, url):
        ...
        info = self._download_json(
            "https://www.rtve.es/api/audios/%s.json" % audio_id, audio_id
        )["page"]["items"][0]
