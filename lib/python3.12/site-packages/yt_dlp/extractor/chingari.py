...
class ChingariBaseIE(InfoExtractor):
    def _get_post(self, id, post_data):
        ...
        song_data = post_data.get(
            "song", {}
        )  # revist this in future for differentiating b/w 'art' and 'author'
        ...
        return {
            ...
            "description": urllib.parse.unquote_plus(
                clean_html(post_data.get("caption"))
            ),
...
class ChingariIE(ChingariBaseIE):
    ...
    def _real_extract(self, url):
        ...
        post_json = self._download_json(
            f"https://api.chingari.io/post/post_details/{id}", id
        )
...
class ChingariUserIE(ChingariBaseIE):
    ...
    def _real_extract(self, url):
        ...
        post_json = self._download_json(
            f"https://api.chingari.io/user/{alt_id}", alt_id
        )
        if post_json["code"] != 200:
            raise ExtractorError(post_json["message"], expected=True)
