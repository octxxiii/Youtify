...
class MusicdexSongIE(MusicdexBaseIE):
    ...
    def _real_extract(self, url):
        ...
        data_json = self._download_json(
            f"https://www.musicdex.org/secure/tracks/{id}?defaultRelations=true", id
        )["track"]
...
class MusicdexAlbumIE(MusicdexBaseIE):
    ...
    def _real_extract(self, url):
        id = self._match_id(url)
        data_json = self._download_json(
            f"https://www.musicdex.org/secure/albums/{id}?defaultRelations=true", id
        )["album"]
        entries = [
...
class MusicdexArtistIE(MusicdexPageIE):
    ...
    def _real_extract(self, url):
        id = self._match_id(url)
        data_json = self._download_json(
            f"https://www.musicdex.org/secure/artists/{id}", id
        )["artist"]
        entries = []
...
class MusicdexPlaylistIE(MusicdexPageIE):
    ...
    def _real_extract(self, url):
        id = self._match_id(url)
        data_json = self._download_json(
            f"https://www.musicdex.org/secure/playlists/{id}", id
        )["playlist"]
        entries = [
            self._return_info(track, track.get("album") or {}, track["id"])
            for track in self._entries(id) or []
            if track.get("id")
        ]
