    ...
    def extract_formats(self, info):
        ...
        for song_format in self._FORMATS:
            ...
            for song in traverse_obj(
                self._call_player_api(song_id, bitrate),
                ("data", lambda _, v: url_or_none(v["url"])),
            ):
                song_url = song["url"]
        ...
        if not formats:
            if err != 0 and (err < 200 or err >= 400):
                raise ExtractorError(
                    f"No media links found (site code {err})", expected=True
                )
            else:
                self.raise_geo_restricted(
                    "No media links found: probably due to geo restriction.",
                    countries=["CN"],
                )
        return formats
...
class NetEaseMusicIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "album_artist": " / ".join(
                traverse_obj(info, ("album", "artists", ..., "name"))
            )
            or None,
            **lyric_data,
            **traverse_obj(
        ...
        if not self._yes_playlist(info["songs"] and program_id, info["mainSong"]["id"]):
            ...
            return {
                ...
                "duration": traverse_obj(
                    info, ("mainSong", "duration", {self.kilo_or_none})
                ),
                **metainfo,
            }
