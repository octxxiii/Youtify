...
class MixchIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            f"https://mixch.tv/u/{video_id}/live", video_id
        )

        initial_js_state = self._parse_json(
            self._search_regex(
                r"(?m)^\s*window\.__INITIAL_JS_STATE__\s*=\s*(\{.+?\});\s*$",
                webpage,
                "initial JS state",
            ),
            video_id,
            ...
            "formats": [
                {
                    "format_id": "hls",
                    "url": (
                        traverse_obj(initial_js_state, ("liveInfo", "hls"))
        ...
        infodict.update(
            {
                ...
                "title": self._html_search_regex(
                    r'class="archive-title">(.+?)</', webpage, "title"
                ),
            }
        )
