...
class MonstercatIE(InfoExtractor):
    ...
    def _extract_tracks(self, table, album_meta):
        for td in re.findall(
            ...
            title = clean_html(
                try_call(
                    lambda: get_element_by_class(
                        "d-inline-flex flex-column", td
                    ).partition(" <span")[0]
                )
            )
            ids = extract_attributes(
                try_call(
                    lambda: get_element_html_by_class(
                        "btn-play cursor-pointer mr-small", td
                    )
                )
                or ""
            )
            ...
            track_number = int_or_none(
                try_call(lambda: get_element_by_class("py-xsmall", td))
            )
            if not track_id or not release_id:
                self.report_warning(f"Skipping track {track_number}, ID(s) not found")
                self.write_debug(
                    f"release_id={repr(release_id)} track_id={repr(track_id)}"
                )
                continue
            yield {
                **album_meta,
                ...
                "artist": clean_html(
                    try_call(lambda: get_element_by_class("d-block fs-xxsmall", td))
                ),
    ...
    def _real_extract(self, url):
        ...
        tracklist_table = (
            try_call(lambda: get_element_by_class("table table-small", html)) or ""
        )
        ...
        date = traverse_obj(
            ...
            (
                {
                    lambda html: get_element_by_class(
                        "font-italic mb-medium d-tablet-none d-phone-block", html
                    ).partition("Released ")
                },
                2,
                {strip_or_none},
        ...
        album_meta = {
            ...
            "album_artist": try_call(
                lambda: get_element_by_class(
                    "h-normal text-uppercase mb-desktop-medium mb-smallish", html
                )
            ),
            "release_date": date,
        }
