...
class VeohIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if not categories:
            category = try_get(
                video, lambda x: x["category"].strip().removeprefix("category_")
            )
...
class VeohUserIE(VeohIE):  # XXX: Do not subclass from concrete IE
    ...
    _TESTS = [
        {
            ...
            "info_dict": {"id": "valentinazoe", "title": "valentinazoe (Uploads)"},
            "playlist_mincount": 75,
        },
        {
            "url": "https://www.veoh.com/users/PiensaLibre",
            "info_dict": {"id": "PiensaLibre", "title": "PiensaLibre (Uploads)"},
            "playlist_mincount": 2,
        },
    ]
    ...
    def _fetch_page(self, uploader, page):
        response = self._download_json(
            "https://www.veoh.com/users/published/videos",
            uploader,
        ...
        for video in response["videos"]:
            yield self.url_result(
                f'https://www.veoh.com/watch/{video["permalinkId"]}',
                VeohIE,
                video["permalinkId"],
                video.get("title"),
            )
    ...
    def _real_initialize(self):
        ...
        self._TOKEN = self._search_regex(
            r'csrfToken:\s*(["\'])(?P<token>[0-9a-zA-Z]{40})\1',
            webpage,
            "request token",
            group="token",
        )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(self._fetch_page, uploader), self._PAGE_SIZE
            ),
            uploader,
            f"{uploader} (Uploads)",
        )
