...
class RTPIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        f = self._parse_json(
            f, video_id, lambda data: self.__unobfuscate(data, video_id=video_id)
        )
        config = self._parse_json(
            config, video_id, lambda data: self.__unobfuscate(data, video_id=video_id)
        )
        ...
        if isinstance(f, dict):
            ...
            if f_dash is not None:
                formats.extend(
                    self._extract_mpd_formats(f_dash, video_id, mpd_id="dash")
                )
        else:
            formats.append(
                {
