...
class PiaproIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        {"url": "https://piapro.jp/content/hcw0z3a169wtemz6", "only_matching": True},
    ]
    ...
    def _real_extract(self, url):
        ...
        str_duration, str_filesize = self._search_regex(
            r"サイズ：</span>(.+?)/\(([0-9,]+?[KMG]?B)）",
            webpage,
            "duration and size",
            group=(1, 2),
            default=(None, None),
        )
        str_viewcount = self._search_regex(
            r"閲覧数：</span>([0-9,]+)\s+", webpage, "view count", fatal=False
        )

        uploader_id, uploader = self._search_regex(
            r'<a\s+class="cd_user-name"\s+href="/(.*)">([^<]+)さん<',
            webpage,
            "uploader",
            group=(1, 2),
            default=(None, None),
        ...
        return {
            ...
            "title": self._html_search_regex(
                r'<h1\s+class="cd_works-title">(.+?)</h1>',
                webpage,
                "title",
                fatal=False,
            ),
            "description": self._html_search_regex(
                r'(?s)<p\s+class="cd_dtl_cap">(.+?)</p>\s*<div',
                webpage,
                "description",
                fatal=False,
            ),
