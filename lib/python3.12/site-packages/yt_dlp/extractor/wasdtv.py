...
class WASDTVBaseIE(InfoExtractor):
    ...
    def _extract_thumbnails(self, thumbnails_dict):
        return [
            ...
            for index, url in enumerate(
                traverse_obj(thumbnails_dict, (("small", "medium", "large"),))
            )
            if url
        ]
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            or self._og_search_title(self._download_webpage(url, video_id)),
            ...
            "view_count": int_or_none(
                stream.get(
                    "stream_current_viewers" if is_live else "stream_total_viewers"
                )
            ),
            "is_live": is_live,
            "formats": formats,
...
class WASDTVStreamIE(WASDTVBaseIE):
    ...
    def _get_container(self, url):
        ...
        channel = self._fetch(
            f"channels/nicknames/{nickname}", video_id=nickname, description="channel"
        )
...
class WASDTVClipIE(WASDTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        formats, subtitles = self._extract_m3u8_formats_and_subtitles(
            clip_data.get("url"), video_id=clip_id, ext="mp4"
        )
        return {
            "id": clip_id,
            "title": clip.get("clip_title")
            or self._og_search_title(self._download_webpage(url, clip_id, fatal=False)),
