...
class TwitterBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        guest_token = self._search_regex(
            r'\.cookie\s*=\s*["\']gt=(\d+);', webpage, "gt", default=None
        ) or self._fetch_guest_token(None)
        headers = {
        ...
        def build_login_json(*subtask_inputs):
            return json.dumps(
                {"flow_token": self._flow_token, "subtask_inputs": subtask_inputs},
                separators=(",", ":"),
            ).encode()
    ...
    def _call_api(self, path, video_id, query={}, graphql=False):
        ...
        headers.update(
            ...
            if self.is_logged_in
            else {"x-guest-token": self._fetch_guest_token(video_id)}
        )
...
class TwitterIE(TwitterBaseIE):
    ...
    def _graphql_to_legacy(self, data, twid):
        result = (
            ...
            if self.is_logged_in
            else traverse_obj(data, ("tweetResult", "result", {dict}), default={})
        )
    ...
    def _call_syndication_api(self, twid):
        ...
        status = self._download_json(
            ...
            query={
                ...
                "token": "".join(
                    random.choices("123456789abcdefghijklmnopqrstuvwxyz", k=10)
                ),
            },
        )
        if not status:
...
class TwitterSpacesIE(TwitterBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if live_status == "is_upcoming":
            ...
            formats = (
                self._extract_m3u8_formats(  # XXX: Some Spaces need ffmpeg as downloader
                    source,
                    metadata["media_key"],
                    "m4a",
                    entry_protocol="m3u8",
