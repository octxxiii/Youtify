...
class NiconicoIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        self._request_webpage(
            "https://account.nicovideo.jp/login", None, note="Acquiring Login session"
        )
        ...
        if "oneTimePw" in page:
            ...
            page = self._download_webpage(
                ...
                data=urlencode_postdata({"otp": self._get_tfa_info("6 digits code")}),
                headers={
                    "Content-Type": "application/x-www-form-urlencoded",
                },
            )
    ...
    def _get_heartbeat_info(self, info_dict):
        ...
        def ping():
            ...
            if tracking_id:
                tracking_url = update_url_query(
                    "https://nvapi.nicovideo.jp/v1/2ab0cbaa/watch", {"t": tracking_id}
                )
        ...
        if dmc_protocol == "http":
            ...
            segment_duration = (
                try_get(
                    self._configuration_arg("segment_duration"), lambda x: int(x[0])
                )
                or 6000
            )
            ...
            data=json.dumps(
                {
                    "session": {
                        ...
                        "protocol": {
                            "name": "http",
                            "parameters": {
                                "http_parameters": {"parameters": protocol_parameters}
                            },
                        },
    ...
    def _extract_format_for_quality(
        ...
        format_id = "-".join(
            [remove_start(s["id"], "archive_") for s in (video_quality, audio_quality)]
            + [dmc_protocol]
        )
        ...
        return {
            ...
            "vcodec": "h264",
            "abr": float_or_none(
                traverse_obj(audio_quality, ("metadata", "bitrate")), 1000
            ),
            "vbr": float_or_none(
                (
                    vid_quality
                    if vid_quality > 0
                    else extract_video_quality(vid_qual_label)
                ),
                1000,
            ),
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            try:
                api_data = self._download_json(
                    "https://www.nicovideo.jp/api/watch/v3/%s?_frontendId=6&_frontendVersion=0&actionTrackId=AAAAAAAAAA_%d"
        ...
        return {
            ...
            "uploader_id": str_or_none(
                traverse_obj(
                    api_data, ("owner", "id"), ("channel", "id"), ("community", "id")
                )
            ),
            "timestamp": parse_iso8601(get_video_info("registeredAt"))
            or parse_iso8601(
                self._html_search_meta(
                    "video:release_date", webpage, "date published", default=None
                )
            ),
    ...
    def _get_subtitles(self, video_id, api_data, session_api_data):
        ...
        thread_ids = traverse_obj(
            api_data, ("comment", "threads", lambda _, v: v["isActive"])
        )
        legacy_danmaku = (
            self._extract_legacy_comments(
                video_id, thread_ids, user_id_str, comment_user_key
            )
            or []
        )
        ...
        new_danmaku = self._extract_new_comments(
            new_comments.get("server"),
            video_id,
            new_comments.get("params"),
            new_comments.get("threadKey"),
        )
    ...
    def _extract_new_comments(self, endpoint, video_id, params, thread_key):
        comments = self._download_json(
            ...
            note="Downloading comments (new)",
            errnote="Failed to download comments (new)",
        )
...
class NiconicoSeriesIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_from_matches(
            traverse_obj(json_data, ("itemListElement", ..., "url")),
            list_id,
            title,
            ie=NiconicoIE,
        )
...
class NiconicoUserIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        list_id = self._match_id(url)
        return self.playlist_result(
            self._entries(list_id), list_id, ie=NiconicoIE.ie_key()
        )
...
class NiconicoLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        ws_url = update_url_query(
            ws_url,
            {
                "frontend_id": traverse_obj(embedded_data, ("site", "frontendId"))
                or "9",
            },
        )
