                ...
                try:
                    ...
                    formats.extend(
                        [
                            {
                                "ext": (
                                    "flv" if rtmp_video_url.startswith("rtmp") else ext
                                ),
                                "url": rtmp_video_url,
...
class MTVItaliaIE(MTVServicesInfoExtractor):
    IE_NAME = "mtv.it"
    _VALID_URL = (
        r"https?://(?:www\.)?mtv\.it/(?:episodi|video|musica)/(?P<id>[0-9a-z]+)"
    )
    _TESTS = [
        {
...
class MTVItaliaProgrammaIE(MTVItaliaIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._get_entries(title, playlist_url), video_id, title, description
        )
