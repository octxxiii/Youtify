...
class TikTokBaseIE(InfoExtractor):
    ...
    def _call_api_impl(
        ...
        return self._download_json(
            "https://%s/aweme/v1/%s/" % (self._API_HOSTNAME, ep),
            video_id=video_id,
            fatal=fatal,
            note=note,
            errnote=errnote,
    ...
    def _call_api(
        ...
        if not self._WORKING_APP_VERSION:
            app_version = self._configuration_arg(
                "app_version", [""], ie_key=TikTokIE.ie_key()
            )[0]
            manifest_app_version = self._configuration_arg(
                "manifest_app_version", [""], ie_key=TikTokIE.ie_key()
            )[0]
            if app_version and manifest_app_version:
                ...
                self.report_warning(
                    "Only one of the two required version params are passed as extractor arguments",
                    only_once=True,
                )
        ...
        if self._WORKING_APP_VERSION:
            ...
            return self._call_api_impl(
                ep, real_query, manifest_app_version, video_id, fatal, note, errnote
            )

        for count, (app_version, manifest_app_version) in enumerate(
            self._APP_VERSIONS, start=1
        ):
            real_query = self._build_api_query(query, app_version, manifest_app_version)
            try:
                res = self._call_api_impl(
                    ep, real_query, manifest_app_version, video_id, fatal, note, errnote
                )
                self._WORKING_APP_VERSION = (app_version, manifest_app_version)
                return res
                ...
                if isinstance(e.cause, json.JSONDecodeError) and e.cause.pos == 0:
                    if count == len(self._APP_VERSIONS):
                        if fatal:
                            ...
                            return
                    self.report_warning(
                        "%s. Retrying... (attempt %s of %s)"
                        % (str(e.cause or e.msg), count, len(self._APP_VERSIONS))
                    )
                    continue
                raise e
    ...
    def _extract_aweme_app(self, aweme_id):
        feed_list = (
            self._call_api(
                ...
                {"aweme_id": aweme_id},
                aweme_id,
                note="Downloading video feed",
                errnote="Unable to download video feed",
            ).get("aweme_list")
            or []
        )
        aweme_detail = next(
            (aweme for aweme in feed_list if str(aweme.get("aweme_id")) == aweme_id),
            None,
        )
        if not aweme_detail:
    ...
    def _get_subtitles(self, aweme_detail, aweme_id):
        ...
        if not subtitles:
            for caption in traverse_obj(
                aweme_detail, ("video", "subtitleInfos", ...), expected_type=dict
            ):
                if not caption.get("Url"):
                    continue
    ...
    def _parse_aweme_video_app(self, aweme_detail):
        ...
        def extract_addr(addr, add_meta={}):
            ...
            if res:
                known_resolutions.setdefault(res, {}).setdefault(
                    "height", add_meta.get("height") or addr.get("height")
                )
                known_resolutions[res].setdefault(
                    "width", add_meta.get("width") or addr.get("width")
                )
        ...
        if auth_cookie:
            for f in formats:
                self._set_cookie(
                    compat_urllib_parse_urlparse(f["url"]).hostname,
                    "sid_tt",
                    auth_cookie.value,
                )
        ...
        user_url = self._UPLOADER_URL_FORMAT % (
            traverse_obj(
                author_info,
                "sec_uid",
                "id",
        ...
        is_generic_og_trackname = music_info.get(
            "is_original_sound"
        ) and music_info.get("title") == "original sound - %s" % music_info.get(
            "owner_handle"
        )
        if is_generic_og_trackname:
            ...
            music_track, music_author = music_info.get("title"), music_info.get(
                "author"
            )
        ...
        return {
            ...
            **traverse_obj(
            ...
            "duration": int_or_none(
                traverse_obj(video_info, "duration", ("download_addr", "duration")),
                scale=1000,
            ),
    ...
    def _parse_aweme_video_web(self, aweme_detail, webpage_url, video_id):
        ...
        author_info = traverse_obj(
            aweme_detail, "authorInfo", "author", expected_type=dict, default={}
        )
        music_info = aweme_detail.get("music") or {}
        stats_info = aweme_detail.get("stats") or {}
        channel_id = traverse_obj(
            author_info or aweme_detail,
            (("authorSecId", "secUid"), {str}),
            get_all=False,
        )
        ...
        for thumb_url in traverse_obj(
            ...
            (
                (None, "video"),
                ("thumbnail", "cover", "dynamicCover", "originCover"),
                {url_or_none},
            ),
        ):
            thumbnails.append(
...
class TikTokIE(TikTokBaseIE):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "params": {
                "skip_download": True
            },  # XXX: unable to download video data: HTTP Error 403: Forbidden
        },
        {
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(
            url, video_id, headers={"User-Agent": "Mozilla/5.0"}
        )
        next_data = self._search_nextjs_data(webpage, video_id, default="{}")
        if next_data:
            status = (
                traverse_obj(
                    next_data, ("props", "pageProps", "statusCode"), expected_type=int
                )
                or 0
            )
            ...
            status = (
                traverse_obj(sigi_data, ("VideoPage", "statusCode"), expected_type=int)
                or 0
            )
            video_data = traverse_obj(
                sigi_data, ("ItemModule", video_id), expected_type=dict
            )
    ...
    def _real_extract(self, url):
        ...
        user_id = (
            self._html_search_regex(
                r"snssdk\d*://user/profile/(\d+)", webpage, "user ID", default=None
            )
            or user_name
        )
        ...
        return self.playlist_result(
            self._entries_api(user_id, videos), user_id, user_name, thumbnail=thumbnail
        )
    ...
    _TESTS = [
        {
            ...
            "info_dict": {"id": "6956990112127585029"},
            "expected_warnings": ["Retrying"],
        },
        {
            ...
            "info_dict": {"id": "7036843036118469381"},
            "expected_warnings": ["Retrying"],
        },
    ]
...
class DouyinIE(TikTokBaseIE):
    ...
    def _real_extract(self, url):
        ...
        render_data = self._search_json(
            r'<script [^>]*\bid=[\'"]RENDER_DATA[\'"][^>]*>',
            webpage,
            "render data",
            video_id,
            contains_pattern=r"%7B(?s:.+)%7D",
            fatal=False,
            transform_source=compat_urllib_parse_unquote,
        )
        if not render_data:
        ...
        return self._parse_aweme_video_web(
            get_first(render_data, ("aweme", "detail")), url, video_id
        )
    ...
    def _real_extract(self, url):
        ...
        if webpage:
            ...
            room_id = (
                traverse_obj(
                    data,
                    ("UserModule", "users", ..., "roomId", {str_or_none}),
                    get_all=False,
                )
                or self._search_regex(
                    r"snssdk\d*://live\?room_id=(\d+)", webpage, "room ID", default=None
                )
                or room_id
            )
