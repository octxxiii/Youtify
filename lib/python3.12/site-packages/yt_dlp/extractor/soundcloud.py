...
class SoundcloudBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        token_verification = Request(
            self._API_VERIFY_AUTH_TOKEN % query, json.dumps(payload).encode("utf-8")
        )
        response = self._download_json(
            token_verification, None, note="Verifying login token...", fatal=False
        )
        if response is not False:
            ...
            self.report_warning(
                "Provided authorization token seems to be invalid. Continue as guest"
            )
