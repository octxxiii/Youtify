...
class KickBaseIE(InfoExtractor):
    def _real_initialize(self):
        self._request_webpage(
            HEADRequest("https://kick.com/"), None, "Setting up session", fatal=False
        )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "channel_id": str_or_none(
                traverse_obj(response, "id", ("livestream", "channel_id"))
            ),
            "uploader": traverse_obj(response, "name", ("user", "username")),
            "uploader_id": str_or_none(
                traverse_obj(response, "user_id", ("user", "id"))
            ),
            "is_live": True,
            "timestamp": unified_timestamp(
                traverse_obj(response, ("livestream", "created_at"))
            ),
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "description": traverse_obj(
                response, ("livestream", "channel", "user", "bio")
            ),
            "channel": traverse_obj(response, ("livestream", "channel", "slug")),
            "channel_id": str_or_none(
                traverse_obj(response, ("livestream", "channel", "id"))
            ),
            "uploader": traverse_obj(
                response, ("livestream", "channel", "user", "username")
            ),
            "uploader_id": str_or_none(
                traverse_obj(response, ("livestream", "channel", "user_id"))
            ),
            "timestamp": unified_timestamp(response.get("created_at")),
            "duration": float_or_none(
                traverse_obj(response, ("livestream", "duration")), scale=1000
            ),
            "thumbnail": traverse_obj(
                response, ("livestream", "thumbnail"), expected_type=url_or_none
            ),
            "categories": traverse_obj(
                response, ("livestream", "categories", ..., "name")
            ),
        }
