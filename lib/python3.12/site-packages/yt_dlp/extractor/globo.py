...
class GloboIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if (
            ...
            and video.get("encrypted") is True
        ):
        ...
        security = self._download_json(
            "https://playback.video.globo.com/v2/video-session",
            video_id,
        ...
        self._request_webpage(
            HEADRequest(security["sources"][0]["url_template"]),
            video_id,
        ...
        signed_md5 = (
            base64.urlsafe_b64encode(hashlib.md5(md5_data).digest()).decode().strip("=")
        )
        ...
        subs = (
            try_get(security, lambda x: x["source"]["subtitles"], expected_type=dict)
            or {}
        )
        ...
        subs = (
            try_get(
                security, lambda x: x["source"]["subtitles_webvtt"], expected_type=dict
            )
            or {}
        )
        for sub_lang, sub_url in subs.items():
