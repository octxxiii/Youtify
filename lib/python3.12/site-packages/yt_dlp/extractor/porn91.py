    ...
    def _real_extract(self, url):
        ...
        if daily_limit:
            raise ExtractorError(
                f"91 Porn says: Daily limit {daily_limit} videos exceeded",
                expected=True,
            )

        video_link_url = self._search_regex(
            r'document\.write\(\s*strencode2\s*\(\s*((?:"[^"]+")|(?:\'[^\']+\'))',
            webpage,
            "video link",
        )
        video_link_url = self._search_regex(
            r'src=["\']([^"\']+)["\']',
            urllib.parse.unquote(video_link_url),
            "unquoted video link",
        )
        ...
        return {
            "id": video_id,
            "title": remove_end(
                self._html_extract_title(webpage).replace("\n", ""),
    ...
    def _get_formats_and_subtitle(self, video_link_url, video_id):
        ...
        if ext == "m3u8":
            formats, subtitles = self._extract_m3u8_formats_and_subtitles(
                video_link_url, video_id, ext="mp4"
            )
        else:
