...
def short_client_name(client_name):
    main, *parts = (
        _split_innertube_client(client_name)[0].replace("embedscreen", "e_s").split("_")
    )
...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    def handle_or_none(self, handle):
        return self._search_regex(
            rf"^({self._YT_HANDLE_RE})$", handle, "@-handle", default=None
        )
    ...
    def _select_api_hostname(self, req_api_hostname, default_client=None):
        return (
            self._configuration_arg("innertube_host", [""], ie_key=YoutubeIE.ie_key())[
                0
            ]
            or req_api_hostname
    ...
    def _extract_api_key(self, ytcfg=None, default_client="web"):
        return self._ytcfg_get_safe(
            ytcfg, lambda x: x["INNERTUBE_API_KEY"], str, default_client
        )
    ...
    def _generate_sapisidhash_header(self, origin="https://www.youtube.com"):
        ...
        sapisidhash = hashlib.sha1(
            f"{time_now} {self._SAPISID} {origin}".encode()
        ).hexdigest()
    ...
    def _call_api(
        ...
        api_key = (
            self._configuration_arg(
                "innertube_key", [""], ie_key=YoutubeIE.ie_key(), casesense=True
            )[0]
            or api_key
            or self._extract_api_key(default_client=default_client)
        )
        ...
        for data in args:
            ...
            sync_ids = (
                try_get(
                    data,
                    (
                        lambda x: x["responseContext"]["mainAppWebResponseContext"][
                            "datasyncId"
                        ],
    ...
    def generate_api_headers(
        ...
        *,
        ...
        headers = {
            ...
            or self._extract_identity_token(ytcfg),
        ...
        if mobj:
            ...
            try:
                return datetime_from_str(
                    "now-%s%s" % (mobj.group("time"), mobj.group("unit"))
                )
            except ValueError:
                return None
...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _comment_entries(
        ...
        max_comments, max_parents, max_replies, max_replies_per_thread, *_ = map(
            lambda p: int_or_none(p, default=sys.maxsize),
            self._configuration_arg(
                "max_comments",
            )
            + [""] * 4,
                    ...
                    [
                        *continuation_items_path,
                        ...
                        (
                            ...
                            if is_first_continuation
                            else ("commentThreadRenderer", "commentRenderer")
                        ),
                    ]
                ]
            try:
    ...
    def _extract_player_response(
        ...
        return (
            self._extract_response(
                ...
                fatal=True,
                default_client=client,
                note="Downloading %s player API JSON"
    ...
    def _get_requested_clients(self, url, smuggled_data):
        ...
        if smuggled_data.get("is_music_url") or self.is_music_url(url):
            requested_clients.extend(
                ...
                if f"{client}_music" in INNERTUBE_CLIENTS
            )
        ...
        while clients:
            ...
            if (
                ...
                if variant == "tv_embedded":
                    ...
                    append_client(
                        f"tv_embedded.{base_client}", f"{base_client}_embedded"
                    )
    ...
    def _needs_live_processing(self, live_status, duration):
        if (
            live_status == "is_live"
            and self.get_param("live_from_start")
            or live_status == "post_live"
            and (duration or 0) > 2 * 3600
        ):
            return live_status
    ...
    def _extract_formats_and_subtitles(
        ...
        for fmt in streaming_formats:
            ...
            language_preference = (
                10
                if audio_track.get("audioIsDefault") and 10
                else (
                    -10
                    if "descriptive" in (audio_track.get("displayName") or "").lower()
                    and -10
                    else -1
                )
            )
            ...
            is_damaged = try_get(
                fmt, lambda x: float(x["approxDurationMs"]) / duration < 500
            )
            if is_damaged:
                self.report_warning(
            ...
            name = (
                fmt.get("qualityLabel") or quality.replace("audio_quality_", "") or ""
            )
            ...
            dct = {
                ...
                "format_note": join_nonempty(
                    join_nonempty(
                        ...
                        language_preference > 0 and " (default)",
                        delim="",
                    ),
                    name,
                ...
                "source_preference": (
                    (-10 if throttled else -5 if itag == "22" else -1)
            ...
            if (all_formats or "dashy" in format_types) and dct["filesize"]:
                yield {
                    **dct,
                    "format_id": (
                        f'{dct["format_id"]}-dashy' if all_formats else dct["format_id"]
                    ),
    ...
    def _real_extract(self, url):
        ...
        if not upload_date or (
            ...
            and "no-youtube-prefer-utc-upload-date"
...
class YoutubeTabBaseInfoExtractor(YoutubeBaseInfoExtractor):
    ...
    def _rich_entries(self, rich_grid_renderer):
        renderer = (
            traverse_obj(
                rich_grid_renderer,
                ("content", ("videoRenderer", "reelItemRenderer", "playlistRenderer")),
                get_all=False,
            )
            or {}
        )
        ...
        if avatar_thumbnails:
            ...
            if uncropped_avatar:
                avatar_thumbnails.append(
                    {"url": uncropped_avatar, "id": "avatar_uncropped", "preference": 1}
                )
        ...
        info.update(
            {
                ...
                "tags": try_get(
                    metadata_renderer or {}, lambda x: x.get("keywords", "").split()
                ),
    ...
    def _extract_inline_playlist(self, playlist, playlist_id, data, ytcfg):
        ...
        for page_num in itertools.count(1):
            ...
            headers = self.generate_api_headers(
                ytcfg=ytcfg,
                account_syncid=self._extract_account_syncid(ytcfg, data),
...
class YoutubeTabIE(YoutubeTabBaseInfoExtractor):
    ...
    def _extract_tab_id_and_name(self, tab, base_url="https://www.youtube.com"):
        ...
        tab_id = (
            tab_url
            and self._get_url_mobj(tab_url)["tab"][1:]
