...
class SkebIE(InfoExtractor):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "subtitles": {"jpn": [{"url": r"re:https://skeb.+", "ext": "vtt"}]},
                "width": 720,
                "height": 405,
                "duration": 313,
                "fps": 30,
    ...
    def _real_extract(self, url):
        ...
        for item in nuxt_data.get("previews") or []:
            ...
            if not preview_ext:
                content_disposition = parse_qs(vid_url)["response-content-disposition"][
                    0
                ]
            ...
            if not vid_url or not item.get("id"):
                continue
            width, height = traverse_obj(item, ("information", "width")), traverse_obj(
        ...
        if not entries:
            raise ExtractorError(
                "No video/audio attachment found in this commission.", expected=True
            )
        elif len(entries) == 1:
            return entries[0]
        else:
