...
class SwearnetEpisodeIE(InfoExtractor):
            ...
            if key == "hls":
                for video_hls in value:
                    fmts, subs = self._extract_m3u8_formats_and_subtitles(
                        video_hls.get("url"), video_id
                    )
                    formats.extend(fmts)
                    self._merge_subtitles(subs, target=subtitles)
            else:
                formats.extend(
                    {"url": video_mp4.get("url"), "ext": "mp4"} for video_mp4 in value
                )
    ...
    def _get_direct_subtitle(self, caption_json):
        ...
        for caption in caption_json:
            subs.setdefault(caption.get("language") or "und", []).append(
                {"url": caption.get("vttUrl"), "name": caption.get("name")}
            )
    ...
    def _real_extract(self, url):
        ...
        json_data = self._download_json(
            f"https://play.vidyard.com/player/{external_id}.json", display_id
        )["payload"]["chapters"][0]

        formats, subtitles = self._get_formats_and_subtitle(
            json_data["sources"], display_id
        )
        self._merge_subtitles(
            self._get_direct_subtitle(json_data.get("captions")), target=subtitles
        )
