        ...
        if not video_url:
            ...
            for file_num, file_element in enumerate(file_elements, start=1):
                ...
                upload_date = unified_strdate(
                    self._search_regex(r"^(\d{8})_", key, "upload date", default=None)
                )
...
class AfreecaTVLiveIE(AfreecaTVIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        for quality_str in self._QUALITIES:
            ...
            stream_base_url = (
                channel_info.get("RMD") or "https://livestream-manager.afreecatv.com"
            )
            stream_info = (
                self._download_json(
                    f"{stream_base_url}/broad_stream_assign.html",
                    broadcast_no,
                    fatal=False,
                    query={
...
class AfreecaTVUserIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://bj\.afreeca(?:tv)?\.com/(?P<id>[^/]+)/vods/?(?P<slug_type>[^/]+)?"
    )
    _TESTS = [
        {
    ...
    def _fetch_page(self, user_id, user_type, page):
        ...
        info = self._download_json(
            f"https://bjapi.afreecatv.com/api/{user_id}/vods/{user_type}",
            user_id,
        ...
        for item in info["data"]:
            yield self.url_result(
                f'https://vod.afreecatv.com/player/{item["title_no"]}/',
                AfreecaTVIE,
                item["title_no"],
            )
