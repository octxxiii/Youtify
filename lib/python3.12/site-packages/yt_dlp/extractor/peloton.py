...
class PelotonIE(InfoExtractor):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "is_live": False,
                "chapters": "count:1",
                "subtitles": {"en": [{"url": r"re:^https?://.+", "ext": "vtt"}]},
            },
            "params": {
                "skip_download": "m3u8",
            },
    ...
    def _real_extract(self, url):
        ...
        metadata = self._download_json(
            "https://api.onepeloton.com/api/ride/%s/details?stream_source=multichannel"
            % video_id,
            video_id,
        )
        ...
        if metadata.get("instructor_cues"):
            subtitles["cues"] = [
                {"data": json.dumps(metadata.get("instructor_cues")), "ext": "json"}
            ]
        ...
        if peloton.get("ride_id"):
            if (
                ...
                return self.url_result(
                    "https://members.onepeloton.com/classes/player/%s"
                    % peloton["ride_id"]
                )
            else:
