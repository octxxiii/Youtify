    ...
    def process_subtitles(vod_data, process_url):
        ...
        for caption in traverse_obj(vod_data, ("captions", "list", ...)):
            ...
            if caption.get("type") == "fan":
                lang += "_fan%d" % next(
                    i for i in itertools.count(1) if f"{lang}_fan{i}" not in ret[type_]
                )
...
class NaverLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        page = self._download_webpage(
            url, video_id, "Downloading Page", "Unable to download Page"
        )
        secure_url = self._search_regex(
            r'sApiF:\s+(?:"|\')([^"\']+)', page, "secureurl"
        )
