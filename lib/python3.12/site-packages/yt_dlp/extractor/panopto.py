...
class PanoptoBaseIE(InfoExtractor):
    ...
    def _parse_fragment(url):
        return {
            k: json.loads(v[0])
            for k, v in compat_urlparse.parse_qs(
                compat_urllib_parse_urlparse(url).fragment
            ).items()
        }
...
class PanoptoIE(PanoptoBaseIE):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "subtitles": {
                    "en-US": [
                        {"ext": "srt", "data": "md5:a3f4d25963fdeace838f327097c13265"}
                    ],
                    "es-ES": [
                        {"ext": "srt", "data": "md5:57e9dad365fd0fbaf0468eac4949f189"}
                    ],
                },
            },
    ...
    def _mark_watched(self, base_url, video_id, delivery_info):
        ...
        if invocation_id and stream_id and duration:
            timestamp_str = (
                f"/Date({calendar.timegm(datetime.now(timezone.utc).timetuple())}000)/"
            )
            data = {
                "streamRequests": [
                    {
    ...
    def _extract_chapters(timestamps):
        ...
        for timestamp in timestamps or []:
            ...
            if not caption or start is None or duration is None:
                continue
            chapters.append(
                {"start_time": start, "end_time": start + duration, "title": caption}
            )
        return chapters
    ...
    def _extract_mhtml_formats(base_url, timestamps):
        ...
        for timestamp in timestamps or []:
            ...
            obj_id, obj_sn = (
                timestamp.get("ObjectIdentifier"),
                timestamp.get("ObjectSequenceNumber"),
            )
            if (
            ...
            if stream_url:
                ...
                if media_type in ("hls",):
                    m3u8_formats, stream_subtitles = (
                        self._extract_m3u8_formats_and_subtitles(stream_url, video_id)
                    )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "thumbnail": base_url
            + f"/Services/FrameGrabber.svc/FrameRedirect?objectId={video_id}&mode=Delivery&random={random()}",
...
class PanoptoListIE(PanoptoBaseIE):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "info_dict": {"id": "panopto_list", "title": "panopto_list"},
