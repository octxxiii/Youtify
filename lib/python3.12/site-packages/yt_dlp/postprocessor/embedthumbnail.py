...
class EmbedThumbnailPP(FFmpegPostProcessor):
    ...
    def _get_thumbnail_resolution(self, filename, thumbnail_dict):
        ...
        try:
            ...
            self.report_warning(
                "unable to find the thumbnail resolution; %s" % error_to_compat_str(err)
            )
            return guess()
        ...
        if info["ext"] == "mp3":
            ...
            if not mutagen or prefer_atomicparsley:
                ...
                try:
                    ...
                    self.report_warning(
                        "unable to embed using mutagen; %s" % error_to_compat_str(err)
                    )
                    success = False
            ...
            if not success:
                ...
                if atomicparsley is None:
                    ...
                    self.write_debug(
                        "AtomicParsley command line: %s" % shell_quote(cmd)
                    )
