...
class MoveFilesAfterDownloadPP(PostProcessor):
    ...
    def run(self, info):
        ...
        make_newfilename = lambda old: decodeFilename(
            os.path.join(finaldir, os.path.basename(encodeFilename(old)))
        )
        for oldfile, newfile in info["__files_to_move"].items():
            if not newfile:
                newfile = make_newfilename(oldfile)
            if os.path.abspath(encodeFilename(oldfile)) == os.path.abspath(
                encodeFilename(newfile)
            ):
                continue
