...
def create_mapping_re(supported):
    return re.compile(
        r"{0}(?:/{0})*$".format(r"(?:\s*\w+\s*>)?\s*(?:%s)\s*" % "|".join(supported))
    )
    ...
    def _get_metadata_opts(self, info):
        ...
        def add(meta_list, info_list=None):
            value = next(
                (
                    str(info[key])
                    for key in [f"{meta_prefix}_"]
                    + list(variadic(info_list or meta_list))
...
class FFmpegSubtitlesConvertorPP(FFmpegPostProcessor):
    ...
    def run(self, info):
        ...
        for lang, sub in subs.items():
            ...
            if ext == new_ext:
                self.to_screen(
                    "Subtitle file for %s is already in the requested format" % new_ext
                )
                continue
            elif ext == "json":
                self.to_screen(
    ...
    def fixup_webp(self, info, idx=-1):
        ...
        if thumbnail_ext:
            if (
                ...
                self.to_screen(
                    'Correcting thumbnail "%s" extension to webp' % thumbnail_filename
                )
